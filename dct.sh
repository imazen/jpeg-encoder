...
# --- Configuration ---
JPEG_FILENAME="jpegli_d1_0_ref.jpg"
PPM_FILENAME="test_gradient.ppm"
PPM_WIDTH=16
PPM_HEIGHT=16
...
command -v g++ >/dev/null 2>&1 || MISSING_TOOLS+="build-essential " # Common meta-package
...
if [ ! -z "$MISSING_TOOLS" ]; then
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        sudo apt-get update && sudo apt-get install -y git cmake build-essential python3 python3-pip
        # Optional: Install Pillow for a more robust extractor (if needed)
...
# --- 2. Clone Jpegli (if necessary) ---
if [ ! -d "jpegli" ]; then
    echo "Cloning jpegli repository..."
    git clone https://github.com/google/jpegli.git
else
    echo "jpegli directory already exists, skipping clone."
fi

# --- 3. Build Jpegli (if necessary) ---
CJPEGLI_PATH="jpegli/build/tools/cjpegli"
if [ ! -f "$CJPEGLI_PATH" ]; then
    echo "Building cjpegli..."
    cd jpegli
    mkdir -p build
    cd build
    cmake .. -DCMAKE_BUILD_TYPE=Release
    make -j$(nproc) cjpegli
    cd ../.. # Back to original directory
    if [ ! -f "$CJPEGLI_PATH" ]; then
        echo "Error: cjpegli build failed."
        exit 1
    fi
    echo "cjpegli built successfully."
else
    echo "cjpegli already built, skipping build."
fi


# --- 4. Create Test Image ---
echo "Creating test PPM image ($PPM_FILENAME)..."
{
    echo "P6" # Magic number for binary RGB PPM
    echo "$PPM_WIDTH $PPM_HEIGHT"
    echo "255" # Max color value
    for ((y=0; y<$PPM_HEIGHT; y++)); do
        for ((x=0; x<$PPM_WIDTH; x++)); do
            val=$(( (x * 255) / (PPM_WIDTH - 1) )) # Simple gradient
            # Write binary RGB values (grayscale)
            printf "\\$(printf %03o $val)\\$(printf %03o $val)\\$(printf %03o $val)"
        done
    done
} > "$PPM_FILENAME"
echo "Test image created."

# --- 5. Encode with cjpegli ---
echo "Encoding $PPM_FILENAME to $JPEG_FILENAME using distance $CJPEGLI_DISTANCE..."
"$CJPEGLI_PATH" --distance "$CJPEGLI_DISTANCE" "$PPM_FILENAME" "$JPEG_FILENAME"
# Or use quality:
# "$CJPEGLI_PATH" --quality "$CJPEGLI_QUALITY" "$PPM_FILENAME" "$JPEG_FILENAME"
echo "Encoding complete."

# --- 6. Create DQT Extractor Script ---
echo "Creating DQT extractor script (extract_dqt.py)..."
...
echo "Extractor script created."

# --- 7. Run Extractor ---
echo "Extracting DQT tables from $JPEG_FILENAME..."
echo "--- Start Extracted Tables (multiplied by 8) ---"
python3 ./extract_dqt.py "$JPEG_FILENAME"
echo "--- End Extracted Tables ---"

echo "Script finished. Please copy the Rust arrays above into jpeg-encoder/src/quantization.rs"
